...
(defn ^:private external-retract-loop
  ...
  [initial-retractions get-alphas-fn memory transport listener]
  (loop [_ true]
      ...
      (when-let [insertions (seq (apply concat (vals token-insertion-map)))]
        ...
        (cond
          ...
          *current-session*
          ...
          ;; Any session implementation is required to bind this during external retractions.
  ...
  (insert [session facts]
    (let [transient-memory (mem/to-transient memory)
      ...
      (binding [*pending-external-retractions* (atom [])]
        ...
        (external-retract-loop facts get-alphas-fn transient-memory transport transient-listener))
